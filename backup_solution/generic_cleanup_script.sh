...
SOURCES= ( /storage/900B-8135/DCIM/ /storage/emulated/0/DCIM/ /storage/emulated/0/Pictures/)
...
##
 ...
 for SOURCE in "${SOURCES[@]}"
    do 	rsync -avR --recursive --backup --backup-dir=backup --suffix=$RANDOM --remove-source "$SSHUSER"@"$FROMSSH":"$SOURCE"  "$TARGET" 
;done
